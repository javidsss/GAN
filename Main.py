...
for epochs in range(num_epoch):
    for Batch_Index, (Real_Image,_) in enumerate(IterationOfTheData):
        ...
        Noise_Rand = torch.rand(batch_size, Noise_Dim).to(device)
        ...
        if Batch_Index % 100 == 0:
            print(f"Batch: [{Batch_Index}/{len(IterationOfTheData)}] \ "
                  f"eEpoch: [{epochs}/{num_epoch}] \ "
                  f"Loss Discriminator: {loss_disc: 0.4f} & Loss Generator: {loss_gen: 0.4f}"
            )

            with torch.no_grad():
                fake_Image = gen(Noise_Rand).reshape(-1, 1, Image_Height, Image_Width)
                ...
                writer_real.add_image(
                    "Fake Images", img_grid_real, global_step=step
                )
